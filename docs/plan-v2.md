...
| WS1a | **Proxmox lab**: Packer `proxmox-iso` + Autounattend.xml Windows templates → Terraform (`bpg/proxmox`) or clone scripts: 1× Server 2022 DC (`gpno.test`), 1× member, 1× Win11 client; post-convergence snapshots for cheap resets; headless GPO authoring via GroupPolicy PS module; `Backup-GPO` export script | Claude authors, **Matt owns/runs** | True critical path; starts immediately |
